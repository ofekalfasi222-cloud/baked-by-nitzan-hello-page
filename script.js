...
function createProductCard(product) {
  ...
  card.innerHTML = `
    ...
    <div class="product-card-content">
      ...
      <div class="product-card-footer">
        ...
        <a href="${createWhatsAppLink(product.name)}" class="product-card-order" target="_blank" rel="noopener" onclick="event.stopPropagation(); typeof gtag==='function'&&gtag('event','whatsapp_order',{product_name:'${product.name.replace(/'/g,"\\'")}'});">
          להזמנה
        </a>
      </div>
    </div>
  ...
  card.addEventListener('click', () => {
    typeof gtag==='function'&&gtag('event','view_product',{product_name:product.name,category:product.category});
    openModal(product);
  });
  return card;
}
...
function openModal(product) {
  ...
  modalOrderBtn.href = createWhatsAppLink(product.name);
  modalOrderBtn.onclick = function() { typeof gtag==='function'&&gtag('event','whatsapp_order',{product_name:product.name,source:'modal'}); };
